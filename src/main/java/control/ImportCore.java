...
public class ImportCore {
	private static final TRLocalizator LOC = new TRLocalizator(ImportCore.class);
			...
			try {
				...
				try (final Workbook workbook = WorkbookFactory.create(importFile)) {
					if (workbook.getNumberOfSheets() < 1) {
						TRMessage.showErrDialog(parent, ImportCore.LOC.getRes("errNoSheetsFound"));
						return;
					}
					...
					TRMessage.showErrDialog(parent, ImportCore.LOC.getRes("errEncryptedDocument"));
					e.printStackTrace();
				}
			} catch (final FileNotFoundException e) {
				TRMessage.showErrDialog(parent, ImportCore.LOC.getRes("errFileNotFoundException"));
				e.printStackTrace();
			} catch (final IOException e) {
				TRMessage.showErrDialog(parent, ImportCore.LOC.getRes("errIOException"));
				e.printStackTrace();
			}
		} finally {
